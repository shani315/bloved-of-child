...
namespace BL
{
    public class VolunteerBL
    {
        ...
        public static bool removeVolunteerByID(int id)
        {
            ...
            //SortedList<string, string> forSendEmail = new SortedList<string, string>();
            //forSendEmail.Add("body", "הוסרת מאתר ידיד לילד מאחר ולא התקבל ממך שירות זמן רב");
            //forSendEmail.Add("subject", "הסרה מהאתר ידיד לילד");
            //forSendEmail.Add("emailAdress", v.mail);
            if (v != null)
            {
        ...
        {
            if (volunteeringET.volunteer.Find(v.mail) == null)
            {
